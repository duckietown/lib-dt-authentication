...
class Scope:
    ...
    def _sanity_check(self):
        ...
        if self.identifier is not None and self.resource is None:
            raise ValueError(
                "If you set the field 'identifier' you must also set the field 'resource'"
            )
    ...
    def parse(cls, scope: Union[str, Dict[str, str]]) -> "Scope":
        if isinstance(scope, str):
            ...
            if len(scope_parts) > 3:
                raise ValueError(
                    "Only scopes in the form '<action>[:<resource>[:<resource-id>]]' "
                    "are supported."
                )
        ...
        if isinstance(scope, dict):
            ...
            raise ValueError(
                f"Scope of type '{type(scope).__name__}' not supported. "
        ...
        if self.service is None:
            return ":".join(
                filter(
                    ...
                    [self.action, self.resource, self.identifier],
                )
            )
    ...
    def __str__(self):
        return (
            json.dumps(self.compact(), sort_keys=True)
