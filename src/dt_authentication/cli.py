...
def cli_generate(args=None):
    ...
    parser.add_argument(
        "--ndays", type=int, default=365, help="Number of days for validity"
    )
    parser.add_argument(
        "--nhours", type=int, default=0, help="Number of hours for validity"
    )
    parser.add_argument(
        "--nminutes", type=int, default=0, help="Number of minutes for validity"
    )
...
def _print_token_info(token: DuckietownToken):
    exp_info: str = (
        "expired"
        if token.expired
        else f"in {(token.expiration - datetime.datetime.now()).days} days"
    )
    print(
        f"""
Payload:
